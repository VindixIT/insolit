...
func CreateClienteHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "POST" {
		...
		sqlStatement := "INSERT INTO features( name, endereco, capacidade, cnpj) VALUES ($1, $2, $3,$4) RETURNING id"
		id := 0
...
func UpdateClienteHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "POST" {
		...
		capacidade := r.FormValue("Capacidade")
		cnpj := r.FormValue("Cnpj")
		sqlStatement := "UPDATE features SET name=$1, endereco=$2, capacidade=$3, cnpj=$4 WHERE id=$5"
...
func DeleteClientesByRoleHandler(roleId string) {
	sqlStatement := "DELETE FROM Clientes WHERE cliente_id=$1"
	deleteForm, err := Db.Prepare(sqlStatement)
	if err != nil {
		panic(err.Error())
	}
	deleteForm.Exec(roleId)
	//log.Println("DELETE clientes in cliente Id: " + clienteId)
}
