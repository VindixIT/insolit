...
func UpdateConcessionariaHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "POST" {
		...
		name := r.FormValue("Name")
		cnpj := r.FormValue("CNPJ")
		sqlStatement := "UPDATE concessionaria SET name=$1, cnpj=$2 WHERE id=$3"
...
func DeleteConcessionariaHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "POST" {
		id := r.FormValue("Id")
		sqlStatement := "DELETE FROM concessionaria WHERE id=$1"
...
func ListConcessionariasHandler(w http.ResponseWriter, r *http.Request) {
	...
	sec.IsAuthenticated(w, r)
	rows, err := Db.Query("SELECT id, name, cnpj FROM concessionarias order by id asc")
	sec.CheckInternalServerError(err, w)
